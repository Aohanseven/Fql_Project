...
def get_storelist():
    df1 = pd.read_sql(
        "SELECT store_id,geo  from store_count_street_point WHERE is_street = 1",
        con)
    df2 = pd.read_sql(
        "SELECT unit_rent,lat,lng  from zu_shop_data_copy_copy WHERE is_ture = 1",
        con)
    for store_id, geo in zip(df1['store_id'], df1['geo']):
        ...
        lng = float(lat_lng[1])
        rent_list = []
        for unit_rent, lat_p, lng_p, in zip(
                df2['unit_rent'], df2['lat'], df2['lng']):
        ...
        cur.execute(
            "insert into street_rent_list(store_id,geo,rent_list) VALUES ('%s','%s','%s')" %
            (store_id, geo, rent_list))
...
def get_storelist1():
    df1 = pd.read_sql(
        "SELECT store_id,geo  from street_rent_list_copy WHERE  rent_list=''",
        con)
    df2 = pd.read_sql(
        "SELECT unit_rent,lat,lng  from zu_shop_data_copy_copy WHERE is_ture = 1",
        con)
    for store_id, geo in zip(df1['store_id'], df1['geo']):
        ...
        rent_list = (','.join(rent_list))
        cur.execute(
            "update street_rent_list_copy set rent_list = '%s' WHERE store_id='%s'" %
            (rent_list, store_id))
        print('sucess id(%s)' % store_id)
        con.commit()
    con.close()
