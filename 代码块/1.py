...
#         print u'搜索结果数量: ', number
        ...
        #                 print self.browser.page_source
                        ...
                        try :
                            ...
                            r = res.strip('0').strip('.') if '.' in res else res
    ...
    def update_info( self,
        ...
        try: 
            ...
            conn = pymysql.connect(host,user,pwd,table,charset='utf8')
        # 使用cursor()方法获取操作游标
            sql_id = int(self.id)
            cursor = conn.cursor()
            if capital_money!='':
...
#     print c.crack(3)[1]
    ...
    db = pymysql.connect(host,user,pwd,table,charset='utf8')
    # 使用cursor()方法获取操作游标
    cursor = db.cursor()
    
    sql = "SELECT id,company_name,capital_money FROM stang_new_evaluate_intelligence e  \
       where e.update_status ='%s' and id < 405253 "% ("")
    cursor.execute(sql)
