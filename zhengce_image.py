    ...
    for i in range(1,6):
        ...
        for url in urls:
            if url != '':
                ...
                new_image_url = 'http://fbd.fangqianli.com/FBDweb/News_pic?img=' + str(int(time.time())) + str(random.randint(1000, 9999)) + '.img'
                id = re.findall(r'http://fbd.fangqianli.com/FBDweb/News_pic\?img=(.*?)\..*', new_image_url)[0]
                conn = pymysql.connect(host='192.168.0.252', user='web_user', passwd='first2018pl,', db='FBDdata2', charset='utf8', port=3306)
                cue = conn.cursor()
                print('mysql connect success')
                try:
                    cue.execute("INSERT INTO news_image(id,image,image_url,new_image_url,image_type) VALUES (%s,%s,%s,%s,%s)",
                                (id, image, image_url, new_image_url, image_type))
